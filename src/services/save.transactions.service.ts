...
): Promise<Pain001 | Pain013 | Pacs008> => {
...
const handlePain001 = async (transaction: Pain001): Promise<Pain001> => {
  ...
  transaction.DebtorAcctId =
    transaction.CstmrCdtTrfInitn.PmtInf.DbtrAcct.Id.Othr.Id;
  transaction.CreditorAcctId =
  ...
  const Amt =
    transaction.CstmrCdtTrfInitn.PmtInf.CdtTrfTxInf.Amt.InstdAmt.Amt.Amt;
  const Ccy =
    transaction.CstmrCdtTrfInitn.PmtInf.CdtTrfTxInf.Amt.InstdAmt.Amt.Ccy;
  ...
  const EndToEndId =
    transaction.CstmrCdtTrfInitn.PmtInf.CdtTrfTxInf.PmtId.EndToEndId;
  const lat =
    transaction.CstmrCdtTrfInitn.SplmtryData.Envlp.Doc.InitgPty.Glctn.Lat;
  const long =
    transaction.CstmrCdtTrfInitn.SplmtryData.Envlp.Doc.InitgPty.Glctn.Long;
...
const handlePain013 = async (transaction: Pain013): Promise<Pain013> => {
  ...
  const Amt =
    transaction.CdtrPmtActvtnReq.PmtInf.CdtTrfTxInf.Amt.InstdAmt.Amt.Amt;
  const Ccy =
    transaction.CdtrPmtActvtnReq.PmtInf.CdtTrfTxInf.Amt.InstdAmt.Amt.Ccy;
  ...
  const creditorAcctId =
    transaction.CdtrPmtActvtnReq.PmtInf.CdtTrfTxInf.CdtrAcct.Id.Othr.Id;
  ...
  try {
    ...
    await cacheDatabaseClient.saveTransactionRelationship(
...
const handlePacs008 = async (transaction: Pacs008): Promise<Pacs008> => {
  ...
  transaction.EndToEndId =
    transaction.FIToFICstmrCdt.CdtTrfTxInf.PmtId.EndToEndId;
  transaction.DebtorAcctId =
    transaction.FIToFICstmrCdt.CdtTrfTxInf.DbtrAcct.Id.Othr.Id;
  transaction.CreditorAcctId =
    transaction.FIToFICstmrCdt.CdtTrfTxInf.CdtrAcct.Id.Othr.Id;
...
export const rebuildCache = async (
  ...
  const currentPain001 = (await databaseClient.getTransactionPain001(
    endToEndId,
  )) as [Pain001[]];
  if (!currentPain001 || !currentPain001[0] || !currentPain001[0][0]) {
    LoggerService.error(
      'Could not find pain001 transaction to rebuild dataCache with',
    );
    return undefined;
  }
  const dataCache: DataCache = {
    cdtrId:
      currentPain001[0][0].CstmrCdtTrfInitn.PmtInf.CdtTrfTxInf.Cdtr.Id?.PrvtId
        .Othr.Id,
    dbtrId:
      currentPain001[0][0].CstmrCdtTrfInitn.PmtInf.Dbtr.Id?.PrvtId.Othr.Id,
    cdtrAcctId:
      currentPain001[0][0].CstmrCdtTrfInitn.PmtInf.CdtTrfTxInf.CdtrAcct.Id.Othr
        .Id,
    dbtrAcctId:
      currentPain001[0][0].CstmrCdtTrfInitn.PmtInf.DbtrAcct.Id.Othr.Id,
  };
